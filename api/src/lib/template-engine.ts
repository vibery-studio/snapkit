...
/**
 * Render custom layout HTML with placeholder replacement.
 * Used directly when we have raw html/css strings from D1.
 */
export function renderCustomLayoutHtml(html: string, css: string, p: LayoutRenderParams): string {
 ...
 */
export function createLayoutFromTemplate(data: CustomLayoutData): Layout & { custom: true } {
  return {
    id: data.id,
    name: data.name,
