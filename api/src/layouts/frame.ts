...
import { sanitizeColor, sanitizeUrlForCss, watermarkHtml } from '../lib/html-helpers';
...
export const frameLayout: Layout = {
  ...
  render(p: LayoutRenderParams): string {
    ...
    // Use typed logo_position from LayoutRenderParams (also accept legacy cast)
    const logoPos = p.logo_position || params.logo_position || 'bottom-right';
    ...
    const hasImage = imgUrl && imgUrl !== 'undefined' && imgUrl !== 'null';
    const imgHtml = hasImage
      ? `<img src="${imgUrl}" alt="" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;display:block;" crossorigin="anonymous" onerror="this.style.display='none'" />`
      : '';
    const bgColor = hasImage ? '#000' : '#ddd';

    return `<div id="thumbnail" style="width:${p.width}px;height:${p.height}px;position:relative;font-family:'Montserrat',sans-serif;overflow:hidden;background:${bgColor};">
  ${imgHtml}
